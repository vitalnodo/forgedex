#!/bin/sh
# Build hello_jni example
# Run from repo root: sh examples/hello_jni/build.sh
...
rm -f classes.dex app.unsigned.apk app.apk libhello.so

# Compile native library
# Note: targeting musl libc, but Android uses bionic — close enough for this purpose
zig cc -target aarch64-linux-musl -Oz -fPIC -shared -I. -o libhello.so hello-jni.c || exit 1

# Assemble DEX
"$FASMG" jni.asm classes.dex || exit 1

# Convert manifest to binary AXML
"$MARCO" < AndroidManifest.xml > /tmp/AndroidManifest.xml || exit 1

# Pack APK — .so must be under lib/arm64-v8a/
mkdir -p lib/arm64-v8a
cp libhello.so lib/arm64-v8a/

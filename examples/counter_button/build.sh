#!/bin/sh
# Build and install counter_button example
# Run from repo root: sh examples/counter_button/build.sh
...
PACKAGE="app.hello.counter_button"
...
"$FASMG" counter_button.asm classes.dex || exit 1
...
if [ "${1}" = "--install" ]; then
    ...
    adb install app.apk
    adb shell am start -n "$PACKAGE/.CounterButton"
else
    echo "Built: app.apk"
